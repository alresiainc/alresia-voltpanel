...
export default function Processes() {
  const [procs, setProcs] = useState<Service[]>([])
  const [form, setForm] = useState({ id:'', name:'', command:'', args:'', cwd:'' })
  const load = () => api.listServices().then(setProcs).catch(() => {})
  useEffect(() => { load() }, [])
  return (
          ...
          {procs.map(p=> (
            <tr key={p.id} className="border-b">
              ...
              <td className="text-right space-x-2">
                <button onClick={()=>api.restartService(p.id).then(load)} className="border px-2">Restart</button>
                <button onClick={()=>api.stopService(p.id).then(load)} className="border px-2">Stop</button>
              </td>
            </tr>
          ))}
        </tbody>
